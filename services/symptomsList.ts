export const symptomsList = [
  "ABASIA",
  "Abdomen",
  "ABORTO",
  "ABSCESO O EMPIEMA (en general)",
  "ABURRIMIENTO",
  "ACCIDENTE",
  "ACCIDENTE CEREBRO – VASCULAR (A.C.V.)",
  "ACIDOSIS",
  "Acné",
  "ACNÉ",
  "ACROMEGALIA",
  "ACROQUERATOSIS",
  "ACUFENO",
  "ADDISON",
  "ADENITIS",
  "ADENOIDES",
  "ADENOPATÍA",
  "ADHERENCIA",
  "ADORMECIMIENTO – TORPOR",
  "ADULTO MAYOR",
  "AEROFAGIA",
  "AFASIA",
  "AFONÍA o EXTINCIÓN DE VOZ",
  "AFTA",
  "AGITACIÓN",
  "AGORAFOBIA",
  "AGOTAMIENTO o BURNOUT",
  "AGRESIVIDAD",
  "AGUJETAS",
  "AHOGOS",
  "ALCOHOLISMO",
  "ALERGIA A LA FIEBRE DEL HENO (resfriado – rinitis alérgica)",
  "ALERGIA A LA LECHE o A LOS PRODUCTOS LÁCTEOS",
  "ALERGIA A LAS PICADURAS DE AVISPAS Y ABEJAS",
  "ALERGIA A LAS PLUMAS",
  "ALERGIA A LOS ANIMALES (en general)",
  "ALERGIA A LOS ANTIBIÓTICOS",
  "ALERGIA A LOS CABALLOS",
  "ALERGIA A LOS FRESONES",
  "ALERGIA A LOS GATOS",
  "ALERGIA A LOS PECES O A LOS FRUTOS DE MAR",
  "ALERGIA A LOS PERROS",
  "ALERGIA AL ACEITE o A LA MANTEQUILLA DE CACAHUETE",
  "ALERGIA AL POLEN",
  "ALERGIA AL POLVO",
  "Alergias",
  "ALERGIAS (en general)",
  "ALEXIA CONGÉNITA (ceguera de las palabras)",
  "ALIENTO (mal)",
  "ALOPECIA",
  "ALUCINACIONES",
  "ALZHEIMER (enfermedad de…)",
  "AMPOLLA",
  "AMPUTACIÓN",
  "AMÍGDALAS – AMIGDALITIS",
  "ANEMIA",
  "ANEURISMA",
  "ANGINA",
  "ANGINA DE PECHO",
  "ANGIOMA",
  "ANGUSTIA",
  "ANO",
  "ANOREXIA",
  "ANORGASMIA",
  "ANOSMIA",
  "ANTRAX",
  "ANURIA",
  "APATÍA",
  "APENDICITIS",
  "APETITO (pérdida de…)",
  "APOPLEJÍA",
  "ARRITMIA",
  "ARTERIAS",
  "ARTERIOSCLEROSIS",
  "ARTICULACIONES",
  "ARTRITIS",
  "ARTROSIS",
  "ASFIXIA",
  "ASMA",
  "ASTIGMATISMO",
  "ATAXIA",
  "ATELECTASIA",
  "AUTISMO",
  "AUTOINMUNE (ENFERMEDAD)",
  "AZÚCAR",
  "BOCA (en general)",
  "BOCA – ENCÍAS (dolores o problemas de las…)",
  "BOCA – LENGUA (dolores o problemas de la…)",
  "BOCA – PALADAR (problemas del…)",
  "BOCIO",
  "BOLAS (en el cuerpo)",
  "BORRACHERA",
  "BRAZO (en general)",
  "BRAZOS",
  "BRENDA",
  "BRONCONEUMONÍA",
  "BRONQUIOS – BRONQUITIS",
  "BRUXISMO",
  "BULIMIA",
  "BURSITIS",
  "CABEZA (dolores de…)",
  "CABEZA – CEFALEAS",
  "CABEZA – MIGRAÑA",
  "CADERAS (dolores o problemas de las…)",
  "CALAMBRE",
  "CALCIO (falta de…)",
  "CALVICIE",
  "CALLOS",
  "CALOR (sofocos)",
  "CANCER – CÁNCER",
  "CÁNCER DE HUESOS",
  "CÁNCER DE LA LENGUA",
  "CÁNCER DE LA PIEL",
  "CÁNCER DE MAMAS",
  "CÁNCER DE ÚTERO",
  "CANDIDIASIS",
  "CANSANCIO – FATIGA",
  "CARA (dolores o problemas de la…)",
  "CARBUNCO",
  "CARDIAS",
  "CARIES DENTALES",
  "CARTÍLAGO",
  "CASPA",
  "CATARATAS",
  "CATARRO",
  "CELULITIS",
  "CEREBELO (problemas del…)",
  "CEREBRO",
  "CEREBRO (tumor cerebral)",
  "CIÁTICA",
  "CICATRIZACIÓN",
  "CIEGO",
  "CIFOSIS",
  "CIGARRILLO (fumar)",
  "CIRCULACIÓN SANGUINEA",
  "CIRROSIS",
  "CISTITIS",
  "CLAUSTROFOBIA",
  "CLAVÍCULA",
  "CODOS (dolores o problemas de los…)",
  "COLESTEROL",
  "COLITIS",
  "COLON – INTESTINO GRUESO",
  "COMA",
  "COMEZÓN",
  "COMPLEJO DE INFERIORIDAD",
  "CONJUNTIVITIS",
  "CONSTIPACIÓN",
  "CONTRARIEDAD",
  "CONVULSIONES",
  "CORAZÓN",
  "CORAZÓN (angina de pecho o infarto)",
  "CORAZÓN – TAQUICARDIA",
  "CORIZA",
  "CORNEA (lesión de la…)",
  "COSTILLAS",
  "COXIS",
  "CRÁNEO",
  "CRISIS EPILEPTICA (ver: EPILEPSIA)",
  "CRINTAS",
  "CUELLO (dolores o rigidez del…)",
  "CUELLO DEL ÚTERO",
  "CUERPO (lado derecho)",
  "CUERPO (lado izquierdo)",
  "Culebrilla",
  "CULPA",
  "CUTIS",
  "DALTONISMO",
  "DEDO GORDO DEL PIE",
  "DEDOS",
  "DELGADEZ",
  "DELIRIUM TREMENS",
  "DEMENCIA SENIL",
  "DENGUE",
  "DENTICIÓN",
  "DEPRESIÓN",
  "DERMATITIS",
  "DERMIS",
  "DESMAYO",
  "DESNUTRICIÓN",
  "DIABETES",
  "DIARREA",
  "DIENTES",
  "DIENTES (dolor de…)",
  "DIGESTIÓN (problemas de…)",
  "DISCO (hernia discal)",
  "DISLEXIA",
  "DISMENORREA",
  "DISENTERÍA",
  "DISPEPSIA",
  "DISTROFIA MUSCULAR",
  "DIVIESO",
  "DOLOR",
  "DOLORES MIGRATORIOS",
  "DORSAL (espina…)",
  "DROGA (adicción a la…)",
  "DUODENO",
  "ECZEMA",
  "EDEMA",
  "EMBARAZO (problemas del…)",
  "EMBOLIA",
  "EMOCIONES (bloqueo de las…)",
  "ENCÉFALO",
  "ENCÍAS",
  "ENDOMETRIOSIS",
  "ENANISMO",
  "ENFERMEDAD DE CROHN",
  "ENFERMEDADES CRÓNICAS",
  "ENFERMEDADES HEREDITARIAS",
  "ENFERMEDADES INFANTILES",
  "ENFERMEDADES MENTALES",
  "ENFERMEDADES VENÉREAS",
  "ENFISEMA PULMONAR",
  "ENFRIAMIENTO",
  "ENTUMECIMIENTO",
  "ENURESIS",
  "ENVEJECIMIENTO",
  "EPILEPSIA",
  "EQUILIBRIO (falta de…)",
  "ERUCTOS",
  "ESCALOFRÍOS",
  "ESCAMAS",
  "ESCARLATINA",
  "ESCLERODERMIA",
  "ESCLEROSIS EN PLACAS O MÚLTIPLE",
  "ESCLEROSIS LATERAL AMIOTRÓFICA (ELA)",
  "ESCOLIOSIS",
  "ESGUINCE",
  "ESÓFAGO",
  "ESPALDA (dolores de…)",
  "ESPASMOFILIA",
  "ESPASMOS",
  "ESPINILLA",
  "ESPONDILITIS ANQUILOSANTE",
  "ESPOLÓN",
  "ESTADO DE ÁNIMO",
  "ESTERILIDAD",
  "ESTÓMAGO",
  "ESTÓMAGO (ardores de…)",
  "ESTRABISMO",
  "ESTRÉS",
  "ESTREÑIMIENTO",
  "EUTANASIA",
  "EXANTEMA",
  "EXTREMIDADES (frías)",
  "FALTA DE APETITO",
  "FALTA DE EQUILIBRIO",
  "FARINGITIS",
  "FATIGA CRÓNICA",
  "FEMUR",
  "FIBROMA",
  "FIBROMIALGIA",
  "FIBROSIS QUÍSTICA",
  "FIEBRE",
  "FIEBRE DE PLÁTICO",
  "FÍSTULA",
  "FLATO",
  "FLEBITIS",
  "FLEMAS",
  "FOBIAS",
  "FORÚNCULO",
  "FRACTURA",
  "FRIGIDEZ",
  "FRÍO",
  "GANGRENA",
  "GANGLIOS LINFÁTICOS",
  "GARGANTA",
  "GARGANTA (dolor, irritación)",
  "GASES",
  "GASTRITIS",
  "GATO (alergia al…)",
  "GEMELOS (músculos)",
  "GENITALES",
  "GINGIVITIS",
  "GLALCOMA",
  "GLÁNDULAS",
  "GLÁNDULAS SUPRARRENALES",
  "GLOBO OCULAR",
  "GLÓBULOS BLANCOS",
  "GLÓBULOS ROJOS",
  "GOLPE",
  "GOTA",
  "GRASA",
  "GRIPE",
  "Garganta",
  "HALITOSIS",
  "HEMORRAGIA",
  "HEMORROIDES",
  "HEPATITIS",
  "HERIDA",
  "HERNIA",
  "HERNIA DE HIATO",
  "HERNIA DISCAL",
  "HERNIA INGUINAL",
  "HERNIA UMBILICAL",
  "HERPES",
  "HERPES GENITAL",
  "HERPES LABIAL",
  "HIDROPESÍA",
  "HIGADO",
  "HIPERACTIVIDAD",
  "HIPERMETROPÍA",
  "HIPERTENSIÓN",
  "HIPERTIROIDISMO",
  "HIPOCLUCEMIA",
  "HIPÓFISIS",
  "HIPOTENSIÓN",
  "HIPOTÁLAMO",
  "HIPOTIROIDISMO",
  "HISTERIA",
  "HOMBROS",
  "HOMOSEXUALIDAD",
  "HONGOS",
  "HORMONAS",
  "HUESOS",
  "HUESOS (cáncer de…)",
  "HUEVOS (alergia a los…)",
  "HUMEDAD",
  "Hígado",
  "ICITERICIA",
  "IDEAS SUICIDAS",
  "IDIOTEZ",
  "IMPACIENCIA",
  "IMPOTENCIA",
  "INCONTINENCIA",
  "INDIGESTIÓN",
  "INFECCIÓN",
  "INFECCIÓN URINARIA",
  "INFERTILIDAD",
  "INFLAMACIÓN",
  "INFLUENZA",
  "INSOMNIO",
  "INSUFICIENCIA RENAL",
  "INSULINA",
  "INTESTINO DELGADO",
  "INTESTINO GRUESO (Colon)",
  "INTOXICACIÓN",
  "IRA",
  "JUANETE",
  "LABIOS",
  "LADO DERECHO DEL CUERPO",
  "LADO IZQUIERDO DEL CUERPO",
  "LARINGITIS",
  "LECHE (alergia a la…)",
  "LENGUA",
  "LEPRA",
  "LESBIANISMO (ver: HOMOSEXUALIDAD)",
  "LEUCEMIA",
  "LEUCORREA",
  "LINFÁTICO (sistema)",
  "LIPOMA",
  "LÍQUIDO (retención de…)",
  "LUMBAGO",
  "LUNARES",
  "LUPUS",
  "LLAGAS",
  "LLANTO",
  "MACULOPATÍA",
  "MADRE (relación con la…)",
  "MAL DE OJO",
  "MALFORMACIÓN",
  "MAMAS (senos)",
  "MAMAS (cáncer de…)",
  "MANCHAS EN LA PIEL",
  "MANDÍBULA",
  "MANOS",
  "MAREOS",
  "MATRIZ",
  "MEDULA ESPINAL",
  "MEDULA ÓSEA",
  "MELANCOLÍA",
  "MELANOMA",
  "MEMORIA (pérdida de…)",
  "MENINGITIS",
  "MENOPAUSIA",
  "MENSTRUACIÓN",
  "MENTE",
  "METABOLISMO",
  "MIEDO",
  "MIELITIS",
  "MIEMBRO FANTASMA",
  "MIGRAÑA",
  "MIOPÍA",
  "MOCO, MUCOSIDAD",
  "MONONUCLEOSIS",
  "MORDERSE LAS UÑAS",
  "MUERTE",
  "MUÑECA",
  "MÚSCULOS",
  "MUSLOS",
  "NARIZ (en general)",
  "NARIZ (hemorragia nasal)",
  "NATURALEZA",
  "NAUSEAS",
  "NERVIO CIÁTICO",
  "NERVIOS",
  "NEUMONÍA",
  "NEURALGIA",
  "NEURASTENIA",
  "NEUROSIS",
  "NIÑEZ",
  "NÓDULOS",
  "NUCA",
  "OBESIDAD",
  "OBSESIÓN",
  "OCLUSIÓN INTESTINAL",
  "ODIO",
  "OIDOS (dolor, otitis)",
  "OIDOS (zumbidos)",
  "OJOS",
  "OJOS (astigmatismo)",
  "OJOS (cataratas)",
  "OJOS (conjuntivitis)",
  "OJOS (estrabismo)",
  "OJOS (glaucoma)",
  "OJOS (hipermetropía)",
  "OJOS (miopía)",
  "OJOS (orzuelo)",
  "OJOS (presbicia)",
  "OJOS (sequedad)",
  "OLVIDO",
  "OMBLIGO",
  "OMBROS",
  "OMOPLATO",
  "ONICOFAGIA",
  "OREJAS",
  "ORGULLO",
  "ORINA",
  "ORZUELO",
  "OSTEOPOROSIS",
  "OTITIS",
  "OVARIOS",
  "PACIENTE",
  "PADRE (relación con el…)",
  "PALADAR",
  "PALPITACIONES",
  "PANADIZO",
  "PANCREAS",
  "PÁNICO",
  "PAPERAS",
  "PARÁLISIS",
  "PARÁLISIS FACIAL",
  "PARANOIA",
  "PARÁSITOS",
  "PARKINSON",
  "PARPADOS",
  "PARTO",
  "PECHOS",
  "PELO (caída, calvicie)",
  "PELVIS",
  "PENE",
  "PENSAMIENTOS",
  "PERDON",
  "PERICARDITIS",
  "PERITONITIS",
  "PESADILLA",
  "PESO (exceso de…)",
  "PESTAÑAS",
  "PEZONES",
  "PICADURAS",
  "PIE DE ATLETA",
  "PIEL",
  "PIEL (acné)",
  "PIEL (cáncer de…)",
  "PIEL (eczemas)",
  "PIEL (psoriasis)",
  "PIEL (urticaria)",
  "PIEL (verrugas)",
  "PIEL (vitíligo)",
  "PIERNAS",
  "PIERNAS (varices)",
  "PIES",
  "PIOJOS",
  "PLAQUETAS",
  "PLEURA",
  "PLEXO SOLAR",
  "POLIPO",
  "POLIO",
  "POLEN",
  "POLVO",
  "PRESIÓN ARTERIAL (alta o baja)",
  "PRÓSTATA",
  "PSORIASIS",
  "PULGAR",
  "PULMONES",
  "PUNTA DE COSTADO",
  "QUEMADURA",
  "QUISTE",
  "QUISTE OVÁRICO",
  "RABIA",
  "RAQUITISMO",
  "RECTO",
  "REGULACIÓN TÉRMICA",
  "RECHAZO",
  "REGLA",
  "RESENTIMIENTO",
  "RESFRIADO",
  "RESPIRACIÓN",
  "REUMA, REUMATISMO",
  "RIÑONES",
  "RODILLAS",
  "RONQUIDOS",
  "ROSÁCEA",
  "ROSTRO",
  "ROTULA",
  "RUBÉOLA",
  "SACRO",
  "SALIVA",
  "SALPULLIDO",
  "SANGRE",
  "SANGRE (anemia)",
  "SANGRE (circulación)",
  "SANGRE (diabetes)",
  "SANGRE (hemorragia)",
  "SANGRE (hipertensión)",
  "SANGRE (hipoglucemia)",
  "SANGRE (leucemia)",
  "SARAMPIÓN",
  "SARNA",
  "SEBORREA",
  "SEDIT (sed)",
  "SENILIDAD",
  "SENO (pecho)",
  "SEQUEDAD",
  "SEXUALIDAD",
  "SIDA",
  "SIFILIS",
  "SINDROME DE DOWN",
  "SINUSITIS",
  "SOFOCOS",
  "SORDERA",
  "SUEÑO",
  "SUICIDIO",
  "SUPRARRENALES",
  "TABAQUISMO",
  "TALLA (baja)",
  "TAQUICARDIA",
  "TARTAMUDEZ",
  "TEJIDOS",
  "TENDINITIS",
  "TENDONES",
  "TENSIÓN ARTERIAL",
  "TESTÍCULOS",
  "TÉTANOS",
  "TICS",
  "TIMIDEZ",
  "TIMO",
  "TIROIDES",
  "TOBILLOS",
  "TOC (trastorno obsesivo compulsivo)",
  "TORTÍCOLIS",
  "TOS",
  "TOXICOMANÍA",
  "TRABAJO DE PARTO",
  "TRAQUEA",
  "TRASTORNOS MENSTRUALES",
  "TRAUMATISMO",
  "TRIGÉMINO (neuralgia del…)",
  "TRISTEZA",
  "TROMBOSIS",
  "TUBERCULOSIS",
  "TUMOR",
  "TÚNEL CARPIANO (bloqueo del…)",
  "Úlcera",
  "ÚLCERA GÁSTRICA o PÉPTICA) (duodeno o estómago)",
  "ÚLCERA(S) (en general)",
  "UMBILICAL (hernia…)",
  "Uñas",
  "UÑAS (comerse las…)",
  "UÑAS (en general)",
  "UÑAS AMARILLAS (síndrome de las…)",
  "UÑAS BLANDAS Y FRÁGILES",
  "UÑERO O UÑA ENCARNADA",
  "UREMIA",
  "URETERITIS",
  "Uretra",
  "URETRITIS",
  "URTICARIA",
  "Útero",
  "ÚTERO (cáncer del cuello del…)",
  "ÚTERO (en general)",
  "VAGINA – COMEZONES VAGINALES",
  "VAGINA (en general)",
  "VAGINAL – HERPES",
  "VAGINALES (espasmos…)",
  "VAGINALES (pérdidas…)",
  "VAGINITIS",
  "VARICELA",
  "VARICES",
  "VEGETACIONES ADENOIDES",
  "VEGETATIVO CRÓNICO (estado…)",
  "Vejiga",
  "VEJIGA – CISTITIS",
  "VEJIGA (dolores de…)",
  "Venas",
  "VENAS – VARICES",
  "VENAS (dolencias en las…)",
  "VENÉREAS (enfermedades…)",
  "VERRUGAS (en general)",
  "Vértebras",
  "VÉRTIGO",
  "Vesícula biliar",
  "VESÍCULA BILIAR",
  "VIENTRE",
  "VIOLACIÓN",
  "VIRUELAS",
  "VIRUS",
  "Vista",
  "VITÍLIGO",
  "VÓMITO DE SANGRE",
  "VÓMITOS",
  "VOZ – RONQUERA",
  "VOZ (perder la…)",
  "Xerostomía",
  "ZONA",
  "Zumbidos (acúfenos)",
  "ZUMBIDOS DE OÍDOS",
  "ZURDO"
];